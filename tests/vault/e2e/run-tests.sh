...
# no external dependency beyond saas vault's own: kind, docker, kubectl, helm, jq, envsubst,
# curl). Takes several minutes.
#
# Set USE_KIND_CLUSTER_FUNCTION=true to exercise the legacy 'kind_cluster' function instead
# (bash-aliases repo). 'bash tests/vault/e2e/run-tests.sh' starts a NON-interactive bash, which
