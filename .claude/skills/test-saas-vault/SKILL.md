...
This repo has two test tiers for `services/vault/` (plus the small `services/gitlab/lib/vault_integration.sh` and `services/minio/lib/vault_integration.sh` apply-side counterparts):
...
The `saas vault integrate minio` / `saas minio integrate vault` handshake (`minio_integration.sh` on both sides, `values/minio-secretstore.yaml.tpl`/`minio-externalsecret.yaml.tpl`) has its own equivalent full round-trip phase, `integrate-vault-full`, in `tests/minio/e2e/run-tests.sh` instead (it needs a real MinIO release, which belongs to that suite): run `tests/minio/e2e/run-tests.sh --only dev-install,integrate-vault-full` after touching anything shared between the two integrations (`integration_common.sh`, the reviewer-ServiceAccount template). The fast, default-suite order-independence proof for `integrate minio` DOES live in this repo's own unit suite, next to `integrate gitlab`'s (see `tests/vault/unit/test-argparse-values.sh`).
...
5. A change touching the MinIO integration (`minio_integration.sh` on either side, or `minio-secretstore.yaml.tpl`/`minio-externalsecret.yaml.tpl`) needs `bash tests/vault/unit/test-argparse-values.sh` first (the `integrate minio` order-independence cases live there, next to `integrate gitlab`'s), then `tests/minio/e2e/run-tests.sh --only dev-install,integrate-vault-full` (heavy, opt-in) before signing off on the live-secret-seeding/rotation path.
