...
The `--object-storage external` path (`minio_integration.sh`, the gitlab-side apply half of the two-command handshake with `saas minio integrate gitlab`) is exercised by `tests/minio/e2e/run-tests.sh --only dev-install,integrate-gitlab-full` instead, not by this suite: it needs a real MinIO release to integrate against, which belongs to `services/minio/`'s own test suite, not gitlab's. Still run this gitlab suite's `dev-install`/`up-down` unchanged after touching `datastore.sh`'s internal/external split, to confirm the DEFAULT (`--object-storage internal`) path has no regression.
...
- Docker running (also used directly by `registry-push-pull`/`reinstall` to run a throwaway, pinned `crane` container from the host; no host-level Docker config changes, no sudo).
...
4. If the change touches `minio_integration.sh` or `datastore.sh`'s internal/external split (`_saas_gitlab_datastore_psql_secret_apply`/`_saas_gitlab_datastore_minio_secrets_apply`/`_saas_gitlab_datastore_minio_internal_apply`): run this suite's `dev-install`/`up-down` to confirm the default (`internal`) path is unaffected, AND `tests/minio/e2e/run-tests.sh --only dev-install,integrate-gitlab-full` to exercise the `external` path for real (see `.claude/skills/test-saas-minio/`).
