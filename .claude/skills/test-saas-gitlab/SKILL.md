...
description: Re-runs the saas gitlab tests (unit + real E2E) after modifying any file under services/gitlab/ or lib/common.sh. Use whenever install.sh, datastore.sh, tls.sh, runner.sh, cluster.sh, ssh.sh, state.sh, or values/*.yaml.tpl change.
---
...
2. `tests/gitlab/e2e/run-tests.sh` — real, creates a disposable `kind` cluster and installs GitLab for real. Takes several minutes. Run this before signing off on any change to the real install (`install.sh`, `datastore.sh`, `tls.sh`, `runner.sh`, `cluster.sh`, `ssh.sh`, or `values/*.yaml.tpl`).

## What the E2E suite checks

1. **dev-install**: a real `saas gitlab install --cluster-mode kind --mode dev --tls self-signed --non-interactive -y` — checks that `curl` against the ingress with the right `Host` header responds 200/302 (not just that the command doesn't fail), that `saas gitlab credentials` prints a real password, and that the `gitlab-runner` `Deployment` ends up with ready replicas.
2. **up-down**: runs `saas gitlab down` (destroys the cluster) followed by `saas gitlab up` (recreates it) on the previous phase's install, and checks that the `root` password persisted in the state is the SAME before and after (the real proof that the cycle doesn't break credentials against data that's already persisted — see CLAUDE.md, "We have to set the initial root password ourselves") and that the ingress serves traffic again after `up`.
3. **ssh-config**: checks that `saas gitlab ssh-config` prints the right block and that the SSH port exposed on the host accepts connections.

`up-down` and `ssh-config` depend on `dev-install` having left the release alive — either run the full suite, or `--only dev-install --keep` before launching just one of the other two phases.
...
bash tests/gitlab/e2e/run-tests.sh                    # full suite
...
3. If the change touches `install.sh`, `datastore.sh`, `tls.sh`, `runner.sh`, `cluster.sh`, `ssh.sh`, or any `values/*.yaml.tpl`: validate first with `helm template` (no real cluster, see CLAUDE.md, "Manual testing" section) and then run the full E2E suite.
